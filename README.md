# Nugget AI cognitive intelliugence App
...
server-side node code. It then initiates `server.js`. In `server.js` we proxy any 
...
ToDo -
0) s3/buckets/mongo-us-east-2-prod bucket has public access
1) Move prod username/password / config URLS to vault from codebase.
2) Mysql db accepts connection from everywhere . turned on for debugging. Disable that
3) Mongo DB isnt password protected
4) Look why direct URL page refesh give 5xx in prod.
5) Use bcryptjs instead of bcrypt . Change plain text password.
6) Change session store form memory store from redistrore . Investigate why local works and prod has issue (5 points)
7) Split Js files for each moduleusing webpack code splitting .Right now its creating one big js file .
8) Fix Warning: Unknown props.
9) Session is dropping on client restart dev code change. Shouldn't be the case.
